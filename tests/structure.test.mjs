...
const expected = {
  "name": "reminders",
  "version": "0.3.6",
  "url": "https://github.com/PedroAVJ/reminders",
  "dependencies": []
};
...
test("the retired Reminders Clipboard product is no longer packaged", async () => {
  await assert.rejects(access(join(root, "clipboard", "Package.swift")));
  await assert.rejects(access(join(root, "clipboard", "install.sh")));
  await assert.rejects(access(join(root, "script", "build_and_run.sh")));
  await assert.rejects(access(join(root, ".codex", "environments", "environment.toml")));
});
